...
/////////////////////////////////////////////////
	...
	function rwfname(){
		if ($("#gdwpm_masuk_perpus_img").length){
			$('#gdwpm_perpus_img_fname').val('');
			var data = $("#hasil input[name='gdwpm_berkas_terpilih[]']:checked");   // mime, name, id, desc, folder, pptis
			var jsondata = Base64.decode(data[0].value);
			var arraydata = jQuery.parseJSON(jsondata);
			if (arraydata[0].indexOf("image") > -1){
				$('#gdwpm_perpus_img_fname').val(arraydata[3]);
				$('#gdwpm_masuk_perpus_img').show();
			}else{
				$('#gdwpm_masuk_perpus_img').hide();
			}
		}
	}
	...
	$("#gdwpm_berkas_masuk_perpus").click(function(){
		...
		var fnamesiap = '';
		if ($("#gdwpm_masuk_perpus_img").length){
			fnamesiap = [$('#gdwpm_opsi_dummy_dirov').val(), $('#gdwpm_perpus_img_fname').val()];// arr [folder, fname]
			fnamesiap = Base64.encode(JSON.stringify(fnamesiap));
		}
	var data = {
		action: 'gdwpm_on_action',
		masuk_perpus: $("input:radio[name='gdwpm_berkas_terpilih[]']:checked").val(),
		ubah_fname: fnamesiap
	};
	...
	function galerimasuk(i, arraydata) {
		setTimeout(function() {
			if(arraydata == 'entek'){
				$('#gdwpm_berkas_masuk_gallery').show();
				$('#gdwpm_add_to_gal_gbr').hide();
				if((i-2) > 0){
				...
				$('#gdwpm_berkas_masuk_gallery').hide();
				$('#gdwpm_add_to_gal_gbr').show();
