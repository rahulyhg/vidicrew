...
define( 'VERSI_GDWPM', '2.4.4' );
define( 'MY_TEXTDOMAIN', 'gdwpm' );
define( 'GD_BASE_PATH', 'https://docs.google.com/uc?id=' );
    ...
    function open_media_window() {
		if (this.window === undefined) {
			...
			this.window.on('select', function() {
				...
				<?php echo 'if (first.url.indexOf("' . implode('") > -1 || first.url.indexOf("', $gdwpm_dummy_fol['dirlist']) . '") > -1){';?>
					var gdfileid = first.filename;
					var parser = document.createElement('a');
					parser.href = first.url;
					var pathList = parser.pathname.split('/');
					if(pathList.length > 5){
						gdfileid = pathList[4];
					}else{
						if( gdfileid.indexOf(".") > -1 ){
							gdfileid = gdfileid.substr(0, gdfileid.lastIndexOf("."));
							if( gdfileid.indexOf("~") > -1 ){
								gdfileid = gdfileid.substr(gdfileid.lastIndexOf("~") + 1);
							}
						}
					}
...
function gdwpm_filter_gbrurl( $url ){
    ...
    if (strpos($url, 'G_D_W_P_M-ImageFile_ID/') !== false) {
		$url = str_replace( $upload_dir['baseurl'] . '/G_D_W_P_M-ImageFile_ID/', GD_BASE_PATH, $url );
		if(strpos($url, GD_BASE_PATH) === false){
			$url = GD_BASE_PATH . substr($url, strrpos($url, '/') + 1);
		}
...
if(isset($_POST['gdwpm_opsi_thumbs_nonce'])){
	...
	if(!wp_verify_nonce( $_POST['gdwpm_opsi_thumbs_nonce'], 'gdwpm_thumbs_nonce' )) {
		...
		if(ctype_digit($_POST['gdwpm_thumbs_width']) && ctype_digit($_POST['gdwpm_thumbs_height'])){
			...
			if(isset($_POST['gdwpm_thumbs_crop']) == 'true'){$gdwpm_thumbs_crop = 'true';}else{$gdwpm_thumbs_crop = 'false';}
...
jQuery(function() {
	...
	jQuery( "#tabs" ).tabs({
		beforeLoad: function( event, ui ) {
			ui.jqXHR.error(function() {
				ui.panel.html(
				"Opening Options tab, please wait.. <p>If this take too long, there's something wrong with your internet connection. :)</p>" );
			});
		}
    });
	...
	jQuery( "#doktabs" ).tabs({
		beforeLoad: function( event, ui ) {
			ui.jqXHR.error(function() {
				ui.panel.html(
				"Opening plugin documentation tab, please wait..<p>If this take too long, there's something wrong with your internet connection. :)</p>" );
			});
		}
    });
 ...
 <?php if (!empty($foldercek)) { ?>
			<div id="tabs-1" class="ui-helper-clearfix">
				...
				<p>
					...
					<?php
						if(!isset($gdwpm_old_version)){$gdwpm_old_version = get_option('gdwpm_version');}
						if($gdwpm_old_version != VERSI_GDWPM){
					?>
					<div class="ui-state-error ui-corner-all" style="padding:0.7em;width:70%;">
					<p><span class="ui-icon ui-icon-alert" style="float:left; margin-right: .3em;"></span>
					<b>Alert:</b> This plugin did not upgrade properly. Please go to <a href="plugins.php">plugins page</a> and try to deactivate and reactivate it.</p>
					</div><br />
					<?php } ?>
						...
						or: <s>https://www.googledrive.com/host/<code><strong>GOOGLE-DRIVE-FILE-ID</strong></code></s>
						<br />
				...
				<div style="display: none" id="gdwpm_masuk_perpus_teks"><p>Pick a file to include it in the Media Library. 
				<?php
					if(!isset($gdwpm_dummy_fol)){ $gdwpm_dummy_fol = get_option('gdwpm_dummy_folder');}
					if(isset($gdwpm_dummy_fol['status']) && $gdwpm_dummy_fol['status'] == 'checked'){
						$txtpref = 'disabled';
						if($gdwpm_dummy_fol['pref'] == 'checked'){$txtpref = '';}
				?>
					<small style="display:none;float:right;" id="gdwpm_masuk_perpus_img">Override default options: 
						Virtual folder: <select style="width: 100px;" name="gdwpm_opsi_dummy_dirov" id="gdwpm_opsi_dummy_dirov">
						...
						Rewrite filename: <input type="text" size="20" name="gdwpm_perpus_img_fname" id="gdwpm_perpus_img_fname" value="" placeholder="new-filename" <?php echo $txtpref;?>> 
					</small>
				<?php }  ?>
					</p><p>
			...
			<div id="tabs-2">
				...
				<p>
					<ul>
						<li id="infopraupload"><dfn>Your Uploaded files will be listed in "Shared with Me" view (https://drive.google.com/?authuser=0#shared-with-me) in the classic Google Drive UI or "Shared with Me" area (https://drive.google.com/drive/shared-with-me) in the new Google Drive UI.
						</dfn></li>
...
<?php
			...
			$gdwpm_satpam_buat_nonce = wp_create_nonce( 'gdwpm_satpam_aplod_berkas' );
				...
				<?php
					if(!isset($gdwpm_dummy_fol)){ $gdwpm_dummy_fol = get_option('gdwpm_dummy_folder');}
					if(isset($gdwpm_dummy_fol['status']) == 'checked'){ 
				?>
					<small style="float:right;">Options for dummy images are Virtual folder: <?php echo $gdwpm_dummy_fol['dirname'];?>, 
					pattern: #<?php echo $gdwpm_dummy_fol['pattern'];?>, and 
					description as filename: <?php echo empty($gdwpm_dummy_fol['pref']) ? " no" : " yes";?>
					</small>
				<?php }  ?>
		...
		//jQuery('#gdwpm_start-upload').hide();
			...
			<?php $gdwpm_folder_tools_nonce = wp_create_nonce( "gdwpm_folder_tools_nonce" ); ?>
				...
				<p style="margin-left:7px;">
					<dfn>If you found that your folder was listed here, but not in your Google Drive "Share with me" area. <br/>
...
<?php }else{ ?>
			<div id="tabs-6">
				...
				<p>
					Once the folder created, your folder will be listed in the "Incoming" or "Shared with me" area <dfn>https://drive.google.com/drive/shared-with-me</dfn>.
				</p>
				<p>
		...
		<div id="gdwpm-albums" style="margin:0 -12px 0 -12px;">
			...
			<div id="gdwpm-albums-3">
				...
				<div style="display: none" id="gdwpm_masuk_gallery_teks"><p>Collect your images to create a new Gallery.</p>
					<p><span style="display: none" id="gdwpm_add_to_gal_gbr">
							<img src="<?php echo plugins_url( '/images/animation/loading-bar-image.gif', __FILE__ );?>" />
						</span>
			...
			<div id="gdwpm-settingtabs-1">
				...
				<form name="gdwpm_isi_akun" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
					...
					<tr>
						...
						<td>: </td>
						<td>
							<input type="text" name="gdwpm_klaen_aidi" value="<?php echo $gdwpm_opt_akun[1];?>"  title="eg: c529d2ab21e09caae12836337905f0761xxxxxxx" size="45">
						</td>
					</tr>
					<tr>
						<td>
						...
						<td>: </td>
						<td>
							<input type="text" name="gdwpm_nama_service" value="<?php echo $gdwpm_opt_akun[2];?>"  title="eg: budi-123@wati-456789.iam.gserviceaccount.com" size="45">
						</td>
					</tr>
					<tr>
						<td>
...
<?php }else{$cek_kunci = 'false';} ?>
	...
	<div>
		<?php
			...
			$gdwpm_url_tab_dokapi = admin_url( 'admin-ajax.php?action=gdwpm_on_action&gdwpm_tabulasi=docapi&gdwpm_tabulasi_nonce=') . $gdwpm_tabulasi_nonce;
			$gdwpm_url_tab_dokserv = admin_url( 'admin-ajax.php?action=gdwpm_on_action&gdwpm_tabulasi=docserv&gdwpm_tabulasi_nonce=') . $gdwpm_tabulasi_nonce;
		?>
		<div id="doktabs" style="margin:0 -12px 0 -12px;">
			<ul>
				...
				<li><a href="<?php echo $gdwpm_url_tab_dokapi; ?>"><span style="float:left" class="ui-icon ui-icon-suitcase"></span>&nbsp;Google Drive API</a></li>
				<li><a href="<?php echo $gdwpm_url_tab_dokserv; ?>"><span style="float:left" class="ui-icon ui-icon-person"></span>&nbsp;Service Account</a></li>
			</ul>
			<div id="doktabs-1">
...
function gdwpm_ijin_masuk_perpus($jenis_berkas, $nama_berkas, $id_berkas, $deskrip_berkas, $jeneng_folder = 'Uncategorized', $img_sizes = '', $metainfo = ''){
	...
	$gdwpm_fol_n_idth = '';
	if(is_array($deskrip_berkas) && isset($deskrip_berkas[0]) && !empty($deskrip_berkas[0])){
		$vir_dir = $deskrip_berkas[0];
		$deskrip_berkas = $deskrip_berkas[1];
	}
	...
	if(strpos($jenis_berkas, 'image') !== false){
		...
		//// rewrite to local
		if($gdwpm_dummy_fol['status'] == 'checked'){
			$pathdum = $gdwpm_dummy_fol['dirname'];
			if(isset($vir_dir) && !empty($vir_dir)){
				$pathdum = $vir_dir;
			}
			$prefixname = substr($nama_berkas, 0, -4);
			if($gdwpm_dummy_fol['pref'] == 'checked' && !empty($deskrip_berkas)){
				$prefixname = $deskrip_berkas;
			}
			if($gdwpm_dummy_fol['pattern'] == '1'){
				$pathdum .= '/' . sanitize_title_with_dashes($prefixname) . '~' . $id_berkas;
				$gdwpm_fol_n_idth = sanitize_title_with_dashes($prefixname) . '-'.$img_meta['thumbWidth'].'x'.$img_meta['thumbHeight'] . '~' . $gdwpm_fol_n_idth;
			}else{
				$pathdum .= '/' . $id_berkas . '/' . sanitize_title_with_dashes($prefixname);
				$gdwpm_fol_n_idth = 'thumbs/' . $gdwpm_fol_n_idth . '/' . sanitize_title_with_dashes($prefixname) . '-'.$img_meta['thumbWidth'].'x'.$img_meta['thumbHeight'];
			}
			$gdwpm_ekst_gbr = explode('/', $jenis_berkas);
			if($gdwpm_ekst_gbr[1] == 'png' || $gdwpm_ekst_gbr[1] == 'gif' || $gdwpm_ekst_gbr[1] == 'bmp'){
				$gdwpm_fol_n_id = $pathdum . '.' . $gdwpm_ekst_gbr[1];
				$gdwpm_fol_n_idth .= '.' . $gdwpm_ekst_gbr[1];
			}else{
				$gdwpm_fol_n_id = $pathdum . '.jpg';
				$gdwpm_fol_n_idth .= '.jpg';
			}
		}else{
			$gdwpm_fol_n_idth = 'uc?id=' . $gdwpm_fol_n_idth;
		}
...
function gdwpm_action_callback() {
	...
	$gdwpm_service = new GDWPMBantuan( $gdwpm_opt_akun[1], $gdwpm_opt_akun[2], $gdwpm_opt_akun[3] );
		...
		if($css_style == 'justified'){
			...
			foreach($postkontenarr as $key => $val){// id, thumbid, flnme | captionimg
				...
				$postkonten .= '<a href="'.GD_BASE_PATH.$indidatadecode[0].'" '.$boxttl.'data-lightbox="'.$unikid.'"><img '.$jusalt.'src="'.GD_BASE_PATH.$indidatadecode[1].'" /></a>';	//justified
			}
		}else{
		...
		if(isset($_POST['ubah_fname']) && !empty($_POST['ubah_fname'])){
			$fnamearr = json_decode(base64_decode($_POST['ubah_fname']), true);
			$gdwpm_berkas_terpilih_arr[3] = $fnamearr;
		}
	...
	}elseif(isset($_POST['gdwpm_cekbok_buang_dummy'])){
		$nonce = $_REQUEST['gdwpm_override_nonce'];
		
		if ( ! wp_verify_nonce( $nonce, 'gdwpm_override_dir' ) ) {
			wp_die('<strong>Oops... failed!</strong>'); 
		} else {
			if($_POST['gdwpm_cekbok_buang_dummy'] == 'gdwpm_images'){
				echo 'This virtual folder cannot be deleted.';
			}else{
				if(!isset($gdwpm_dummy_fol)){ $gdwpm_dummy_fol = get_option('gdwpm_dummy_folder');}
				$targetbuangvirdir = $_POST['gdwpm_cekbok_buang_dummy'];
				if($gdwpm_dummy_fol['dirname'] == $targetbuangvirdir){$gdwpm_dummy_fol['dirname'] = 'gdwpm_images';}
				if(($konci = array_search($targetbuangvirdir, $gdwpm_dummy_fol['dirlist'])) !== false) {
					unset($gdwpm_dummy_fol['dirlist'][$konci]);
				}
				update_option('gdwpm_dummy_folder', $gdwpm_dummy_fol);	
				echo $targetbuangvirdir . ' has been deleted!';
			}
		}
		...
		if ( ! wp_verify_nonce( $nonce, 'gdwpm_override_dir' ) ) {
			...
			if(!isset($gdwpm_dummy_fol)){ $gdwpm_dummy_fol = get_option('gdwpm_dummy_folder');}
			$dirdef = $_POST['gdwpm_opsi_dummy_dirdef'];
			$dirbaru = sanitize_title_with_dashes($_POST['gdwpm_opsi_dummy_dirname']);
			$gdwpm_dummy_fol['status'] = $_POST['gdwpm_cekbok_opsi_dummy'];
			if($_POST['gdwpm_cekbok_opsi_dummy'] == 'checked'){
				$gdwpm_dummy_fol['pattern'] = $_POST['gdwpm_opsi_dummy_pattern'];
				$gdwpm_dummy_fol['pref'] = $_POST['gdwpm_opsi_dummy_pref'];
				if(isset($_POST['gdwpm_opsi_dummy_dirname']) && !empty($_POST['gdwpm_opsi_dummy_dirname'])){
					if(!in_array($dirbaru, $gdwpm_dummy_fol['dirlist'])){
						$gdwpm_dummy_fol['dirname'] = $dirbaru;
						$gdwpm_dummy_fol['dirlist'][] = $dirbaru;
						update_option('gdwpm_dummy_folder', $gdwpm_dummy_fol);	
						echo $dirbaru . ' has been created and selected as a dummy folder. Option saved!';
					}else{
						echo $dirbaru.' already exists. Nothing to save.';
					}
				}else{
					$gdwpm_dummy_fol['dirname'] = $dirdef;
					update_option('gdwpm_dummy_folder', $gdwpm_dummy_fol);	
					echo $dirdef.' as default dummy folder. Option saved';
				}
			}else{
				update_option('gdwpm_dummy_folder', $gdwpm_dummy_fol);	
				echo 'Option saved!';
		...
		}elseif($_REQUEST['gdwpm_tabulasi'] == 'docapi'){
			$nonce = $_REQUEST['gdwpm_tabulasi_nonce'];
			if ( ! wp_verify_nonce( $nonce, 'gdwpm_tabulasi_ajax' ) ) {
				wp_die('<div class="error"><p>Oops.. security check is not ok!</p></div>'); 
			} else {
				require_once 'google-drive-wp-media-docapi.php';
			}
		}elseif($_REQUEST['gdwpm_tabulasi'] == 'docserv'){
			$nonce = $_REQUEST['gdwpm_tabulasi_nonce'];
			if ( ! wp_verify_nonce( $nonce, 'gdwpm_tabulasi_ajax' ) ) {
				wp_die('<div class="error"><p>Oops.. security check is not ok!</p></div>'); 
			} else {
				require_once 'google-drive-wp-media-docservice.php';
			}
				...
				$daftarfile =  '<div id="'.$div_id.'"><table id="box-table-a" summary="File Folder" class="'.$id_tblpagi.'"><thead><tr><th scope="col"><span class="ui-icon ui-icon-check"></span></th><th scope="col">File ID</th><th scope="col">Title</th><!--<th scope="col">Description</th>--><th scope="col">Size</th><th scope="col">Action</th></tr></thead>';
					...
					foreach ($children->getItems() as $child) {
						...
						if(strpos($file_mime, 'image') !== false){
							$view = '<a href="'. GD_BASE_PATH .$fileId.'" title="Open link in a new window" target="_blank" class="tabeksen">View</a>';
							...
							if(count($getfile_pptis) > 0){
								...
								$img_meta = array_combine($matches[1], $matches[2]);
								if(array_key_exists('thumbId', $img_meta)){
									$file_thumb = GD_BASE_PATH . $img_meta['thumbId'];
								}
							}
						} 
...
function gdwpm_image_rw( $wp ) {
    if ( ! is_admin() && is_404() ) {
		if(!isset($gdwpm_dummy_fol)){$gdwpm_dummy_fol = get_option('gdwpm_dummy_folder');}
		if(!isset($gdwpm_dummy_fol['dirlist']) || empty($gdwpm_dummy_fol['dirlist']) || !is_array($gdwpm_dummy_fol['dirlist'])){
			$gdwpm_dummy_fol['dirlist'] = array('gdwpm_images');
			$gdwpm_dummy_fol['dirname'] = 'gdwpm_images';
			update_option('gdwpm_dummy_folder', $gdwpm_dummy_fol);
		}
		foreach($gdwpm_dummy_fol['dirlist'] as $dir){
			if(strpos($wp->request, '/' . $dir .'/') !== false && strpos($wp->request, '.') !== false){
				$imgbrs = explode('/' . $dir .'/', $wp->request);
				$imgbr = $imgbrs[1];
				if(strpos($imgbr, '/') !== false){
				// format 2 = gdid/imgname.jpg
					if(strpos($imgbr, '/thumbs/') !== false){
						$imgbrs = explode('/thumbs/', $imgbr);
						$imgbr = $imgbrs[1];
					}
					$gdid = substr($imgbr, 0, strpos($imgbr, '/'));
				}else{
				// format 1 = imgname~gdid.jpg
					$gdwpm_ekst_gbr = substr($imgbr, -3);
					$gdid = substr($imgbr, 0, -4);
					if(strpos($gdid, '~') !== false){$gdid = substr($gdid, strrpos($gdid, '~') + 1);}
				}
				// rw img
				$gdwpm_ekst_gbr = substr($imgbr, -3);
				if($gdwpm_ekst_gbr == 'png' || $gdwpm_ekst_gbr == 'gif' || $gdwpm_ekst_gbr == 'bmp'){
					header("Content-Type: image/" . $gdwpm_ekst_gbr);
				}else{
					header("Content-Type: image/jpg");
				}
				$gdurl = "https://docs.google.com/uc?id=" . $gdid;
				@header ('Cache-Control:public, max-age=31536000');
				@header ('Last-Modified: ' . gmstrftime("%a, %d %b %Y %H:%M:%S GMT", time() + 31536000));
				@readfile($gdurl);
				exit;
			}
		}
...
function gdwpm_make_update(){
	$gdwpm_old_version = get_option('gdwpm_version'); 
	if($gdwpm_old_version != VERSI_GDWPM){
		update_option('gdwpm_version', VERSI_GDWPM);
		$gdwpm_dummy_fol_old = get_option('gdwpm_dummy_folder'); 
		if(!is_array($gdwpm_dummy_fol_old)){
			$gdwpm_dummy_fol = array('status'=>'checked', 'dirname'=>'gdwpm_images', 'dirlist'=>array('gdwpm_images'), 'pattern'=>'2', 'pref'=>'checked');
			update_option('gdwpm_dummy_folder', $gdwpm_dummy_fol);
		}else{
			//cek folder 
			foreach($gdwpm_dummy_fol_old['dirlist'] as $dir){
				$upload_dir = wp_upload_dir();
				$targetDir = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . $dir;
				if (file_exists($targetDir)) {
					//savety 1st, rename only.. delete manually
					@rename($targetDir . '/gdwpmrw.php', $targetDir . '/gdwpmrw.php-GDWPM-delete-manually');	//unlink($targetDir . '/gdwpmrw.php');
					@rename($targetDir . '/.htaccess', $targetDir . '/.htaccess-GDWPM-delete-manually');//unlink($targetDir . '/.htaccess');
					@rename($targetDir . '/index.php', $targetDir . '/index.php-GDWPM-delete-manually');
					@rename($upload_dir['basedir'] . DIRECTORY_SEPARATOR . $dir, $upload_dir['basedir'] . DIRECTORY_SEPARATOR . $dir . '-GDWPM-delete-manually');
				}
			}
			$gdwpm_dummy_fol_old['pattern'] = '1';
			update_option('gdwpm_dummy_folder', $gdwpm_dummy_fol_old);
		}
	}
}

add_action( 'plugins_loaded', 'gdwpm_make_update');
